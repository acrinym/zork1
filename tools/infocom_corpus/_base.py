...
SURFACES = {"room-title", "room-description", "object-name", "object-description", "parser-refusal", "death", "action-response", "actor-dialogue", "institutional-document", "manual", "sample-transcript", "hint-question", "hint-answer", "unclassified-player-visible"}
PLAYER_STRING_HEADS = {"DESC", "LDESC", "FDESC", "TEXT", "TELL", "TELL-CR", "PRINT", "PRINTI", "PRINTB", "DPRINT"}
...
_REFERENCE = r"(?:p(?:age)?\.?\s*\d+(?:[-–]\d+)?|line\s+\d+(?:[-–]\d+)?|surface:[a-z0-9._-]+|block:[a-z0-9._-]+)"
SAFE_REFERENCE_RE = re.compile(rf"^{_REFERENCE}(?:\s*(?:,|/)\s*{_REFERENCE})*$", re.IGNORECASE)

FUNCTION_WORDS = {"a", "an", "and", "are", "as", "at", "be", "because", "been", "but", "by", "can", "could", "did", "do", "does", "for", "from", "had", "has", "have", "he", "her", "him", "his", "i", "if", "in", "into", "is", "it", "its", "may", "might", "must", "no", "not", "of", "on", "or", "our", "she", "should", "so", "some", "than", "that", "the", "their", "them", "then", "there", "these", "they", "this", "those", "to", "was", "we", "were", "what", "when", "where", "which", "who", "will", "with", "would", "you", "your"}
...
PUNCTUATION_KEYS = {"comma", "semicolon", "colon", "dash", "parenthetical_open", "question", "exclamation", "ellipsis"}
ANNOTATION_REQUIRED_KEYS = {"schema_version", "word_count", "sentence_count", "sentence_words_mean", "sentence_words_max", "type_token_ratio", "function_word_ratio", "second_person_count", "second_person_present", "fragment_likely", "punctuation", "sensory_channels", "narrator_stance_markers", "parser_behavior", "comedy_mechanics_markers"}
...
def sha256_file(path: Path) -> str:
    """Hash a file without decoding it."""
    return _sha256_bytes(path.read_bytes())


def stable_json_sha(value: Any) -> str:
    """Hash deterministic compact JSON."""
...
def corpus_digest(records: Sequence[Mapping[str, Any]]) -> str:
    """Return the canonical record-id and text-hash corpus digest."""
    return stable_json_sha([{"record_id": row.get("record_id"), "text_sha256": row.get("text_sha256")} for row in records])


def read_json(path: Path) -> Any:
    """Read strict UTF-8 JSON with normalized errors."""
    try:
...
def write_json(path: Path, value: Any) -> None:
    """Write reviewable UTF-8 JSON."""
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(value, indent=2, sort_keys=True) + "\n", encoding="utf-8")


def read_jsonl(path: Path) -> list[dict[str, Any]]:
    """Read object-only strict UTF-8 JSONL."""
...
def write_jsonl(path: Path, rows: Iterable[Mapping[str, Any]]) -> None:
    """Write object rows as UTF-8 JSONL."""
    path.parent.mkdir(parents=True, exist_ok=True)
...
def validate_artifact(artifact: Mapping[str, Any]) -> None:
    ...
    if not _require(artifact, "title", str, artifact_id):
    ...
    if _require(edition, "certainty", str, f"{artifact_id}.edition") not in {"repository-observed", "catalog-confirmed", "family-confirmed", "unresolved"}:
    ...
    if _require(source, "kind", str, f"{artifact_id}.source") not in {"repository", "physical-local", "external-catalog", "permissioned-preservation", "unknown"}:
    ...
    if local_path is not None and (not isinstance(local_path, str) or not Path(local_path).as_posix().startswith(".local/infocom-corpus/")):
        raise CorpusError(f"{artifact_id}.source.local_source_path must remain below .local/infocom-corpus/")
    ...
    if blob_sha is not None and (not isinstance(blob_sha, str) or not GIT_SHA_RE.fullmatch(blob_sha)):
    ...
    if _require(processing, "transcription_status", str, f"{artifact_id}.processing") not in TRANSCRIPTION_STATUSES:
        raise CorpusError(f"{artifact_id}.processing.transcription_status is invalid")
    if _require(processing, "annotation_status", str, f"{artifact_id}.processing") not in ANNOTATION_STATUSES:
    ...
    if public_output not in {"full-text-and-analysis", "statistics-hashes-and-references", "metadata-only"}:
        raise CorpusError(f"{artifact_id}.processing.public_output is invalid")
    if not full_text and public_output == "full-text-and-analysis":
...
def validate_manifest(manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    source = next(item for item in artifacts if item["artifact_id"] == selected_id)
    if source["artifact_type"] != "game-source":
        raise CorpusError("selected source artifact must have artifact_type game-source")
    if source["source"].get("repository_path") != entrypoint:
        raise CorpusError("selected source entrypoint must match source artifact repository_path")
    order = _require(manifest, "authority_order", list, "manifest")
    ...
    if len(order) < 6 or len(set(order)) != len(order):
        raise CorpusError("manifest.authority_order must contain six or more unique tiers")
    return {
        "corpus_id": corpus_id,
        "artifact_count": len(artifacts),
        "rights_counts": {key: sum(1 for item in artifacts if item["rights"]["class"] == key) for key in sorted(RIGHTS_CLASSES)},
        "full_text_artifact_count": sum(1 for item in artifacts if item["rights"]["full_text_allowed"]),
...
def ensure_output_policy(output: Path, repo_root: Path, artifact: Mapping[str, Any]) -> None:
    """Fail closed unless output is local-only or full-text rights are verified."""
    try:
        relative = output.resolve().relative_to(repo_root.resolve())
        under_local = relative.parts[:2] == (".local", "infocom-corpus")
    except ValueError:
        under_local = False
    if under_local or rights_allow_text_export(artifact):
        return
    raise CorpusError(

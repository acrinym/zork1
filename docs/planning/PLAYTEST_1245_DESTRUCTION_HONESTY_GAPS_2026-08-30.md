...
5. **`LIE DOWN`** is in the 1228 syntax patch and README; the Bedroom rejected the sentence. Bare `LIE` works as a zero-object verb. `LIE DOWN` must use `<SYNTAX LIE DOWN OBJECT (FIND RMUNGBIT) = V-HOUSE-SLEEP>` like `LOOK DOWN`; a zero-object `LIE DOWN` line compiles but does not parse. `LIE ON BED` is the object form. ZILF cannot compile `LIE DOWN ON OBJECT` (MDL0112: two prepositions).

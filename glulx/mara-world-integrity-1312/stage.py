#!/usr/bin/env python3
"""Apply Release 1312 over an exact staged Release 1311 source tree."""
...
def validate_manifest(manifest: dict[str, Any], manifest_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(rel, str) or not rel:
        raise RuntimeError("Release 1312 must declare Release 1311 base manifest")
    base = load_json((manifest_path.parent / rel).resolve())
    art = base.get("expected_artifact") if isinstance(base, dict) else None
    if not isinstance(base, dict) or base.get("release") != 1311 or manifest.get("base_release") != 1311:
        raise RuntimeError("Release 1312 base release drift")
    if not isinstance(art, dict) or art.get("locked") is not True:
        raise RuntimeError("Release 1312 requires locked Release 1311 artifact")
    if art.get("sha256") != manifest.get("base_artifact_sha256"):
        raise RuntimeError("Release 1312 base artifact drift")
    return base

def validate_base_source(src: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    receipt = load_json(src / "STAGING-RECEIPT.json")
    if not isinstance(receipt, dict) or receipt.get("release") != 1311:
        raise RuntimeError("base source is not staged Release 1311")
    ...
    if not isinstance(expected_hashes, dict):
        raise RuntimeError("Release 1312 must declare base source identities")
    expected = expected_hashes.get(profile)
    if not isinstance(expected, str) or not expected:
        raise RuntimeError(f"Release 1312 must declare a {profile} base source identity")
    if actual != expected:
        raise RuntimeError(f"Release 1311 {profile} source identity drift: expected {expected}, got {actual}")
    return receipt
...
def main() -> int:
    ...
    try:
        ...
        for name in manifest.get("added_files") or []:
            ...
            if rel.is_absolute() or ".." in rel.parts:
                raise RuntimeError(f"invalid Release 1312 added file: {name}")
            source = (manifest_path.parent / rel).resolve()
            target = (destination / rel).resolve()
            try:
                target.relative_to(destination)
            except ValueError as exc:
                raise RuntimeError(f"invalid Release 1312 added file: {name}") from exc
    ...
    print(f"Staged Release 1312 over locked Release 1311; changed {len(changed)} path(s).")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_release1312: {exc}", file=sys.stderr)

#!/usr/bin/env python3
"""Apply Release 1250 Player Ingenuity / Systemic Workarounds to locked Release 1249."""
...
def validate_manifest(manifest: dict[str, Any], manifest_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(base_value, str) or not base_value:
        raise RuntimeError("Release 1250 must declare its Release 1249 base manifest")
    ...
    if not isinstance(base, dict) or base.get("release") != manifest.get("base_release"):
        raise RuntimeError("Release 1250 base release drift")
    if not isinstance(artifact, dict) or artifact.get("locked") is not True:
        raise RuntimeError("Release 1250 requires a locked Release 1249 artifact")
    if artifact.get("sha256") != manifest.get("base_artifact_sha256"):
        raise RuntimeError("Release 1250 base artifact drift")
    return base
...
def validate_base_source(base_source: Path, base_release: Any, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(identities, dict):
        raise RuntimeError("Release 1250 must pin Release 1249 staged source identities")
    profile = "dev" if bool(receipt.get("dev_mode")) else "production"
    expected = identities.get(profile)
    if not isinstance(expected, str) or not expected:
        raise RuntimeError(f"Release 1250 has no pinned Release 1249 {profile} source identity")
    actual = source_identity(base_source)
    if actual != expected:
        raise RuntimeError(
            f"Release 1249 {profile} source identity drift: expected {expected}, got {actual}"
        )
    return receipt
...
def main() -> int:
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("Release 1250 patch-series must contain an object")
    ...
    print(
        f"Staged Player Ingenuity Release {manifest.get('release')} ({profile}) "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_player_ingenuity: {exc}", file=sys.stderr)

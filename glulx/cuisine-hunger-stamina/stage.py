#!/usr/bin/env python3
"""Stage Cuisine, Hunger, and Stamina over qualified Release 1234."""
...
def validate_base_manifest(
    ...
    """Resolve and validate the exact Release 1234 base contract."""
    ...
    if base.get("release") != manifest.get("base_release"):
        raise RuntimeError("cuisine base release drift")
    if actual_sha != manifest.get("base_artifact_sha256"):
        raise RuntimeError("cuisine base artifact drift")
    return base_path, base


def main() -> int:
    """Stage the exact cuisine delta and write a receipt."""
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("cuisine patch-series must contain an object")
    ...
    stage_base = Path(__file__).resolve().parents[1] / "mara-companion" / "stage.py"
    try:
        subprocess.run(
            [
                sys.executable,
    ...
    print(
        f"Staged Cuisine Release {manifest.get('release')} over "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_cuisine_hunger_stamina: {exc}", file=sys.stderr)

#!/usr/bin/env python3
"""Apply Release 1248 Forest Consequence Physicality to the locked Release 1247 base."""
...
def validate_manifest(manifest: dict[str, Any], manifest_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(base_value, str) or not base_value:
        raise RuntimeError("Release 1248 must declare its Release 1247 base manifest")
    ...
    if not isinstance(base, dict) or base.get("release") != manifest.get("base_release"):
        raise RuntimeError("Release 1248 base release drift")
    if not isinstance(artifact, dict) or artifact.get("locked") is not True:
        raise RuntimeError("Release 1248 requires a locked Release 1247 artifact")
    if artifact.get("sha256") != manifest.get("base_artifact_sha256"):
        raise RuntimeError("Release 1248 base artifact drift")
    return base
...
def main() -> int:
    """Stage Release 1248 and record an exact source-change receipt."""
    ...
    print(
        f"Staged Forest Consequence Physicality Release {manifest.get('release')} "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_forest_consequences: {exc}", file=sys.stderr)

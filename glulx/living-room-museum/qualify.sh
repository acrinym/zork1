...
assert layer['expected_artifact'] == {
    'file': 'zork1-glulx-living-room-museum.ulx',
    'glulx_version_hex': '0x00030103',
    'size_bytes': int(os.environ['MUSEUM_SIZE']),
    'checksum_hex': os.environ['MUSEUM_CHECKSUM'],
    'sha256': os.environ['MUSEUM_SHA256'],
}
PY
...
assert receipt['base']['artifact_sha256'] == 'e0de2b66453e6539370377691486a133ad32b3d53d2ff3e676d0d90f23be0e0f'
...
assert story['size_bytes'] == int(os.environ['MUSEUM_SIZE'])
assert story['checksum_hex'] == os.environ['MUSEUM_CHECKSUM']
assert story['checksum_valid'] is True
assert story['sha256'] == os.environ['MUSEUM_SHA256']
PY
...
serial = json.loads(Path('glulx/build/living-room-museum/SERIAL-NORMALIZATION.json').read_text(encoding='utf-8'))
receipt = {

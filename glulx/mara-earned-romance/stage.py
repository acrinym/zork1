#!/usr/bin/env python3
"""Apply Release 1306 over an exact staged Release 1305 source tree."""
...
def validate_manifest(manifest: dict[str, Any], manifest_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(rel, str) or not rel:
        raise RuntimeError("Release 1306 must declare Release 1305 base manifest")
    base = load_json((manifest_path.parent / rel).resolve())
    art = base.get("expected_artifact") if isinstance(base, dict) else None
    if not isinstance(base, dict) or base.get("release") != 1305 or manifest.get("base_release") != 1305:
        raise RuntimeError("Release 1306 base release drift")
    if not isinstance(art, dict) or art.get("locked") is not True:
        raise RuntimeError("Release 1306 requires locked Release 1305 artifact")
    if art.get("sha256") != manifest.get("base_artifact_sha256"):
        raise RuntimeError("Release 1306 base artifact drift")
    return base

def validate_base_source(src: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    receipt = load_json(src / "STAGING-RECEIPT.json")
    if not isinstance(receipt, dict) or receipt.get("release") != 1305:
        raise RuntimeError("base source is not staged Release 1305")
    ...
    if isinstance(expected, str) and expected and actual != expected:
        raise RuntimeError(f"Release 1305 {profile} source identity drift: expected {expected}, got {actual}")
    return receipt
...
def main() -> int:
    ...
    for name in manifest.get("added_files") or []:
        ...
        if rel.is_absolute() or ".." in rel.parts or target.exists() or not source.is_file():
            raise RuntimeError(f"invalid Release 1306 added file: {name}")
    ...
    print(f"Staged Release 1306 over locked Release 1305; changed {len(changed)} path(s).")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_release1306: {exc}", file=sys.stderr)

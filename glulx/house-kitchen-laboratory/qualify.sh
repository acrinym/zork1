...
assert artifact == {
    'file': os.environ['KITCHEN_FILE'],
    'glulx_version_hex': os.environ['KITCHEN_GLULX_VERSION'],
    'size_bytes': int(os.environ['KITCHEN_SIZE']),
    'checksum_hex': os.environ['KITCHEN_CHECKSUM'],
    'sha256': os.environ['KITCHEN_SHA256'],
}
PY
...
"$GLAZER_BIN" "$ASSEMBLY" -o "$BUILD/$KITCHEN_FILE" \
  2>&1 | tee "$BUILD/glazer-assemble.log"
python glulx/tools/verify_ulx.py "$BUILD/$KITCHEN_FILE" \
...
artifact = Path('glulx/build/house-kitchen-laboratory') / os.environ['KITCHEN_FILE']
assert story['file'] == os.environ['KITCHEN_FILE']
assert story['version_hex'] == os.environ['KITCHEN_GLULX_VERSION']
assert story['size_bytes'] == int(os.environ['KITCHEN_SIZE'])
assert story['checksum_hex'] == os.environ['KITCHEN_CHECKSUM']
assert story['checksum_valid'] is True
assert story['sha256'] == os.environ['KITCHEN_SHA256']
assert artifact.stat().st_size == int(os.environ['KITCHEN_SIZE'])
assert hashlib.sha256(artifact.read_bytes()).hexdigest() == os.environ['KITCHEN_SHA256']
PY
...
Path('glulx/build/house-kitchen-laboratory/QUALIFICATION-RECEIPT.json').write_text(

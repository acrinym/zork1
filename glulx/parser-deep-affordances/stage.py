#!/usr/bin/env python3
"""Stage Parser Deep Affordances over qualified Release 1231."""
...
from stage_assistance import remove_destination
...
def validate_base_manifest(
    ...
    """Resolve and validate the exact Release 1231 base contract."""
    value = manifest.get("base_manifest")
    if not isinstance(value, str) or not value:
        raise RuntimeError("parser affordance manifest must declare base_manifest")
...
def main() -> int:
    """Stage the exact two-path parser delta and write a receipt."""
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("parser affordance patch-series must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)

    base_destination = destination.parent / "base-corpus-warning-src"
    ...
    stage_base = (
        Path(__file__).resolve().parents[1] / "corpus-causal-warning" / "stage.py"
    )
    try:
        subprocess.run(
            [
    ...
    print(
        f"Staged Parser Deep Affordances Release {manifest.get('release')} "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_parser_deep_affordances: {exc}", file=sys.stderr)

#!/usr/bin/env python3
"""Stage museum songbird correspondence over qualified Release 1239."""
...
def validate_base_manifest(
    ...
    if not isinstance(value, str) or not value:
        raise RuntimeError("songbird correspondence manifest must declare base_manifest")
    ...
    try:
        ...
        raise RuntimeError(
            f"songbird correspondence base manifest escaped Glulx lineage root: {base_path}"
        ) from exc
    if not base_path.is_file():
        raise RuntimeError(f"songbird correspondence base manifest is missing: {base_path}")
    ...
    if base.get("release") != manifest.get("base_release"):
        raise RuntimeError("songbird correspondence base release drift")
    if actual_sha != manifest.get("base_artifact_sha256"):
        raise RuntimeError("songbird correspondence base artifact drift")
...
def main() -> int:
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("songbird correspondence patch-series must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)

    base_destination = destination.parent / f"base-museum-ecology-src-{uuid.uuid4().hex}"
    ...
    print(
        f"Staged Museum Songbird Correspondence Release {manifest.get('release')} over "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_museum_songbird_correspondence: {exc}", file=sys.stderr)

#!/usr/bin/env python3
"""Stage the Mara Companion Expedition Foundation over qualified Release 1242."""
...
def validate_base_manifest(
    ...
    if not isinstance(value, str) or not value:
        raise RuntimeError("Mara manifest must declare base_manifest")
    ...
    try:
        ...
        raise RuntimeError(f"Mara base manifest escaped Glulx lineage root: {base_path}") from exc
    ...
    if actual_sha != manifest.get("base_artifact_sha256"):
        raise RuntimeError("Mara base artifact drift")
...
def main() -> int:
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("Mara patch-series must contain an object")

    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)
    base_destination = destination.parent / f"base-mara-src-{uuid.uuid4().hex}"
    ...
    stage_base = Path(__file__).resolve().parents[1] / "natural-play-regressions" / "stage.py"
    try:
        subprocess.run(
            [
                sys.executable,
        ...
        changed = {
            p for p in set(base_files) | set(final_files)
    ...
    print(
        f"Staged Mara Companion Expedition Foundation Release {manifest.get('release')} "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_mara_companion_foundation: {exc}", file=sys.stderr)

#!/usr/bin/env python3
"""Apply Release 1253 Dam Survival & Prepared Rescue to locked Release 1252."""
...
def validate_manifest(manifest: dict[str, Any], manifest_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(base_value, str) or not base_value:
        raise RuntimeError("Release 1253 must declare its Release 1252 base manifest")
    ...
    if not isinstance(base, dict) or base.get("release") != manifest.get("base_release"):
        raise RuntimeError("Release 1253 base release drift")
    if not isinstance(artifact, dict) or artifact.get("locked") is not True:
        raise RuntimeError("Release 1253 requires a locked Release 1252 artifact")
    if artifact.get("sha256") != manifest.get("base_artifact_sha256"):
        raise RuntimeError("Release 1253 base artifact drift")
    return base
...
def validate_base_source(base_source: Path, base_release: Any, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(identities, dict):
        raise RuntimeError("Release 1253 must pin Release 1252 staged source identities")
    profile = "dev" if bool(receipt.get("dev_mode")) else "production"
    expected = identities.get(profile)
    if not isinstance(expected, str) or not expected:
        raise RuntimeError(f"Release 1253 has no pinned Release 1252 {profile} source identity")
    actual = source_identity(base_source)
    if actual != expected:
        raise RuntimeError(
            f"Release 1252 {profile} source identity drift: expected {expected}, got {actual}"
        )
    return receipt
...
def main() -> int:
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("Release 1253 patch-series must contain an object")
    ...
    print(
        f"Staged Dam Survival & Prepared Rescue Release {manifest.get('release')} ({profile}) "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_dam_survival: {exc}", file=sys.stderr)

#!/usr/bin/env python3
"""Apply Release 1258 Mara Causal Biography & Shared Danger to locked Release 1257."""
...
def validate_manifest(manifest: dict[str, Any], manifest_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(base_value, str) or not base_value:
        raise RuntimeError("Release 1258 must declare its Release 1257 base manifest")
    ...
    if not isinstance(base, dict) or base.get("release") != manifest.get("base_release"):
        raise RuntimeError("Release 1258 base release drift")
    if not isinstance(artifact, dict) or artifact.get("locked") is not True:
        raise RuntimeError("Release 1258 requires a locked Release 1257 artifact")
    if artifact.get("sha256") != manifest.get("base_artifact_sha256"):
        raise RuntimeError("Release 1258 base artifact drift")
    return base
...
def validate_base_source(
    ...
    if not isinstance(identities, dict):
        raise RuntimeError("Release 1258 must pin Release 1257 staged source identities")
    ...
    if not isinstance(expected, str) or not expected:
        raise RuntimeError(f"Release 1258 has no pinned Release 1257 {profile} source identity")
    if actual != expected:
        raise RuntimeError(
            f"Release 1257 {profile} source identity drift: expected {expected}, got {actual}"
        )
    return receipt
...
def main() -> int:
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("Release 1258 patch-series must contain an object")
    ...
    for name in manifest.get("added_files") or []:
        ...
        if relative.is_absolute() or ".." in relative.parts:
            raise RuntimeError(f"invalid Release 1258 added file path: {name}")
        ...
        try:
            ...
            raise RuntimeError(
                f"Release 1258 added file escaped staging roots: {name}"
            ) from exc

        if target.exists():
            raise RuntimeError(f"Release 1258 added file already exists in base: {name}")
        if not source_file.is_file():
            raise RuntimeError(f"Release 1258 added file missing: {source_file}")
    ...
    print(
        f"Staged Mara Causal Biography & Shared Danger Release {manifest.get('release')} "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_mara_causal_biography: {exc}", file=sys.stderr)

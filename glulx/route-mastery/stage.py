#!/usr/bin/env python3
"""Apply Release 1252 Earned Sequence Breaks & Route Mastery to locked Release 1251."""
...
def validate_manifest(manifest: dict[str, Any], manifest_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(base_value, str) or not base_value:
        raise RuntimeError("Release 1252 must declare its Release 1251 base manifest")
    ...
    if not isinstance(base, dict) or base.get("release") != manifest.get("base_release"):
        raise RuntimeError("Release 1252 base release drift")
    if not isinstance(artifact, dict) or artifact.get("locked") is not True:
        raise RuntimeError("Release 1252 requires a locked Release 1251 artifact")
    if artifact.get("sha256") != manifest.get("base_artifact_sha256"):
        raise RuntimeError("Release 1252 base artifact drift")
    return base
...
def validate_base_source(base_source: Path, base_release: Any, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(identities, dict):
        raise RuntimeError("Release 1252 must pin Release 1251 staged source identities")
    profile = "dev" if bool(receipt.get("dev_mode")) else "production"
    expected = identities.get(profile)
    if not isinstance(expected, str) or not expected:
        raise RuntimeError(f"Release 1252 has no pinned Release 1251 {profile} source identity")
    actual = source_identity(base_source)
    if actual != expected:
        raise RuntimeError(
            f"Release 1251 {profile} source identity drift: expected {expected}, got {actual}"
        )
    return receipt
...
def main() -> int:
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("Release 1252 patch-series must contain an object")
    ...
    print(
        f"Staged Earned Sequence Breaks & Route Mastery Release {manifest.get('release')} ({profile}) "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_route_mastery: {exc}", file=sys.stderr)

...
def validate_manifest(
    ...
    if not isinstance(base_value, str) or not base_value:
        raise RuntimeError(
            "Release 1264 must declare its Release 1263 base manifest"
        )
    ...
    if not isinstance(base, dict) or base.get("release") != manifest.get("base_release"):
        raise RuntimeError("Release 1264 base release drift")
    if not isinstance(artifact, dict) or artifact.get("locked") is not True:
        raise RuntimeError("Release 1264 requires a locked Release 1263 artifact")
    if artifact.get("sha256") != manifest.get("base_artifact_sha256"):
        raise RuntimeError("Release 1264 base artifact drift")
...
def validate_base_source(
    ...
    if expected != actual:
        raise RuntimeError(
            f"Release 1263 {profile} source identity drift: "
...
def main() -> int:
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("Release 1264 patch-series must contain an object")
    ...
    for name in manifest.get("added_files") or []:
        ...
        if (
            ...
            raise RuntimeError(f"invalid Release 1264 added file: {name}")
    ...
    print(
        f"Staged Perilous Affordances Release {manifest.get('release')} over "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_perilous_affordances: {exc}", file=sys.stderr)

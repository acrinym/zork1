#!/usr/bin/env python3
"""Stage Corpus-Coupled Causal Warning over qualified Release 1230."""
...
def validate_base_manifest(
    ...
    """Resolve and validate the exact Release 1230 base contract."""
    value = manifest.get("base_manifest")
    if not isinstance(value, str) or not value:
        raise RuntimeError("causal warning manifest must declare base_manifest")
    ...
    if base.get("release") != expected_release:
        raise RuntimeError(
            f"base release drift: expected {expected_release}, got {base.get('release')}"
        )
...
def main() -> int:
    """Stage the exact three-path production delta and write a receipt."""
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("causal warning patch-series manifest must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)

    base_destination = destination.parent / "base-completed-expedition-src"
    ...
    stage_base = Path(__file__).resolve().parents[1] / "completed-expedition-archive" / "stage.py"
    try:
        subprocess.run(
            [
                sys.executable,
        ...
        for item in manifest.get("overrides") or []:
            if not isinstance(item, dict):
                raise RuntimeError("each causal warning override must be an object")
    ...
    print(
        f"Staged Corpus-Coupled Causal Warning Release {manifest.get('release')} "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_corpus_causal_warning: {exc}", file=sys.stderr)

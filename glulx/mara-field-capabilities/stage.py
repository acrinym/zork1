#!/usr/bin/env python3
"""Apply Release 1259 Mara Field Capability Discovery to locked Release 1258."""
...
def validate_manifest(manifest: dict[str, Any], manifest_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(base_value, str) or not base_value:
        raise RuntimeError("Release 1259 must declare its Release 1258 base manifest")
    ...
    if not isinstance(base, dict) or base.get("release") != manifest.get("base_release"):
        raise RuntimeError("Release 1259 base release drift")
    if not isinstance(artifact, dict) or artifact.get("locked") is not True:
        raise RuntimeError("Release 1259 requires a locked Release 1258 artifact")
    if artifact.get("sha256") != manifest.get("base_artifact_sha256"):
        raise RuntimeError("Release 1259 base artifact drift")
    return base
...
def validate_base_source(
    ...
    if not isinstance(identities, dict):
        raise RuntimeError("Release 1259 must pin Release 1258 staged source identities")
    ...
    if not isinstance(expected, str) or not expected:
        raise RuntimeError(f"Release 1259 has no pinned Release 1258 {profile} source identity")
    if actual != expected:
        raise RuntimeError(
            f"Release 1258 {profile} source identity drift: expected {expected}, got {actual}"
        )
    return receipt
...
def main() -> int:
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("Release 1259 patch-series must contain an object")
    ...
    for name in manifest.get("added_files") or []:
        ...
        if relative.is_absolute() or ".." in relative.parts:
            raise RuntimeError(f"invalid Release 1259 added file path: {name}")
        ...
        try:
            ...
            raise RuntimeError(f"Release 1259 added file escaped staging roots: {name}") from exc
        if target.exists():
            raise RuntimeError(f"Release 1259 added file already exists in base: {name}")
        if not source_file.is_file():
            raise RuntimeError(f"Release 1259 added file missing: {source_file}")
    ...
    print(
        f"Staged Mara Field Capability Discovery Release {manifest.get('release')} "
        f"({profile}) over Release {base_manifest.get('release')}; changed {len(changed)} path(s)."
    )
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_mara_field_capabilities: {exc}", file=sys.stderr)

#!/usr/bin/env python3
"""Apply Release 1305 over an exact staged Release 1303 source tree."""
...
def validate_manifest(manifest: dict[str, Any], manifest_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(rel, str) or not rel:
        raise RuntimeError("Release 1305 must declare Release 1303 base manifest")
    base = load_json((manifest_path.parent / rel).resolve())
    art = base.get("expected_artifact") if isinstance(base, dict) else None
    if not isinstance(base, dict) or base.get("release") != 1303 or manifest.get("base_release") != 1303:
        raise RuntimeError("Release 1305 base release drift")
    if not isinstance(art, dict) or art.get("locked") is not True:
        raise RuntimeError("Release 1305 requires locked Release 1303 artifact")
    if art.get("sha256") != manifest.get("base_artifact_sha256"):
        raise RuntimeError("Release 1305 base artifact drift")
    return base

def validate_base_source(src: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    receipt = load_json(src / "STAGING-RECEIPT.json")
    if not isinstance(receipt, dict) or receipt.get("release") != 1303:
        raise RuntimeError("base source is not staged Release 1303")
    ...
    if not isinstance(expected, str) or not expected:
        raise RuntimeError(f"Release 1305 {profile} predecessor source identity is not locked; candidate is {actual}")
    if actual != expected:
        raise RuntimeError(f"Release 1303 {profile} source identity drift: expected {expected}, got {actual}")
    return receipt
...
def main() -> int:
    ...
    for name in manifest.get("added_files") or []:
        ...
        if rel.is_absolute() or ".." in rel.parts or target.exists() or not source.is_file():
            raise RuntimeError(f"invalid Release 1305 added file: {name}")
    ...
    print(f"Staged Release 1305 over locked Release 1303; changed {len(changed)} path(s).")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_release1305: {exc}", file=sys.stderr)

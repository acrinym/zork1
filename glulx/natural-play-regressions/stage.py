#!/usr/bin/env python3
"""Stage natural-play regression repairs over qualified Release 1241."""
...
def validate_base_manifest(manifest: dict[str, Any], manifest_path: Path) -> tuple[Path, dict[str, Any]]:
    ...
    if not isinstance(value, str) or not value:
        raise RuntimeError("natural-play manifest must declare base_manifest")
    ...
    try:
        ...
        raise RuntimeError(f"natural-play base manifest escaped Glulx lineage root: {base_path}") from exc
    if not base_path.is_file():
        raise RuntimeError(f"natural-play base manifest is missing: {base_path}")
    ...
    if base.get("release") != manifest.get("base_release"):
        raise RuntimeError("natural-play base release drift")
    if actual_sha != manifest.get("base_artifact_sha256"):
        raise RuntimeError("natural-play base artifact drift")
...
def main() -> int:
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("natural-play patch-series must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)
    base_destination = destination.parent / f"base-natural-play-src-{uuid.uuid4().hex}"
    ...
    stage_base = Path(__file__).resolve().parents[1] / "museum-troll-provenance" / "stage.py"
    try:
        subprocess.run([
        ...
        receipt = {
            "edition": manifest.get("edition"), "release": manifest.get("release"),
            ...
            "overrides": overrides, "patches": applied, "changed_paths": sorted(changed),
    ...
    print(f"Staged Natural Play Regressions Release {manifest.get('release')} over Release {base_manifest.get('release')}; changed {len(changed)} path(s).")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_natural_play_regressions: {exc}", file=sys.stderr)

#!/usr/bin/env python3
"""Stage the evolving-house state foundation over qualified Release 1218."""
...
def validate_base_manifest(
    ...
    """Verify exact derivation from the qualified Release 1218 manifest."""
    value = manifest.get("base_manifest")
    if not isinstance(value, str) or not value:
        raise RuntimeError("house-state manifest must declare base_manifest")
...
def main() -> int:
    """Stage the exact base, apply the reviewed house delta, and emit a receipt."""
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("house-state patch-series manifest must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)

    base_destination = destination.parent / "base-room-density-src"
    ...
    stage_base = Path(__file__).resolve().with_name("stage_room_density.py")
    try:
        subprocess.run(
            [
                sys.executable,
        ...
        for item in manifest.get("overrides") or []:
            if not isinstance(item, dict):
                raise RuntimeError("each house-state override must be an object")
    ...
    remove_destination(base_destination)
    print(
        f"Staged Glulx house-state foundation Release {manifest.get('release')} over "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_house_state_foundation: {exc}", file=sys.stderr)

#!/usr/bin/env python3
"""Stage Attic NPC Dossiers over qualified Release 1224."""
...
def validate_base_manifest(
    ...
    if not isinstance(value, str) or not value:
        raise RuntimeError("NPC dossier manifest must declare base_manifest")
...
def main() -> int:
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("NPC dossier patch-series manifest must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)

    base_destination = destination.parent / "base-attic-archive-core-src"
    ...
    stage_base = Path(__file__).resolve().with_name("stage_attic_archive_core.py")
    try:
        subprocess.run(
            [
                sys.executable,
        ...
        for item in manifest.get("overrides") or []:
            if not isinstance(item, dict):
                raise RuntimeError("each NPC dossier override must be an object")
    ...
    remove_destination(base_destination)
    print(
        f"Staged Glulx Attic NPC Dossiers Release {manifest.get('release')} "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_attic_npc_dossiers: {exc}", file=sys.stderr)

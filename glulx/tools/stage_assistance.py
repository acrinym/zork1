#!/usr/bin/env python3
"""Stage the Release 121 assistance layer over Tara's pinned Glulx tree."""
...
def apply_added_file(
    spec: dict[str, Any], manifest_path: Path, destination: Path
) -> dict[str, Any]:
    """Add one reviewed repository-owned source file to the staged tree."""
    relative = safe_relative(str(spec.get("path", "")), manifest_path)
    source_path = (manifest_path.parent / str(spec.get("source", ""))).resolve()
    if not source_path.is_file():
        raise RuntimeError(f"added source is missing: {source_path}")
    target = destination / relative
    if target.exists():
        raise RuntimeError(f"added source target already exists: {relative}")
...
def validate_base_manifest(manifest: dict[str, Any], manifest_path: Path) -> dict[str, Any]:
    """Verify that this layer explicitly derives from the qualified Release 1201 manifest."""
    value = manifest.get("base_manifest")
    if not isinstance(value, str) or not value:
        raise RuntimeError("assistance manifest must declare base_manifest")
...
def main() -> int:
    """Stage, patch, verify the layer boundary, and write a deterministic receipt."""
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("assistance patch-series manifest must contain an object")
    ...
    if not expected_tree or tree != expected_tree:
        raise RuntimeError(
    ...
    print(
        f"Staged assistance Release {manifest.get('release')} from base "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_assistance: {exc}", file=sys.stderr)
        raise SystemExit(2)

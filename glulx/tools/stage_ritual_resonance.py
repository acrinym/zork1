#!/usr/bin/env python3
"""Stage focused Glulx ritual resonance over qualified Release 1215."""
...
def validate_base_manifest(
    ...
    """Verify exact derivation from the qualified Release 1215 manifest."""
    value = manifest.get("base_manifest")
    if not isinstance(value, str) or not value:
        raise RuntimeError("ritual-resonance manifest must declare base_manifest")
...
def main() -> int:
    """Stage the exact base, apply the ritual delta, and emit a receipt."""
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("ritual-resonance patch-series manifest must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)

    base_destination = destination.parent / "base-dam-mechanisms-src"
    ...
    stage_base = Path(__file__).resolve().with_name("stage_dam_mechanisms.py")
    try:
        subprocess.run(
            [
                sys.executable,
        ...
        for item in manifest.get("overrides") or []:
            if not isinstance(item, dict):
                raise RuntimeError("each ritual-resonance override must be an object")
    ...
    remove_destination(base_destination)
    print(
        f"Staged Glulx ritual resonance Release {manifest.get('release')} over "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_ritual_resonance: {exc}", file=sys.stderr)

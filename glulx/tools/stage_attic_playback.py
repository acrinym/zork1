#!/usr/bin/env python3
"""Stage Attic Playback over qualified Release 1226."""
...
def validate_base_manifest(
    ...
    if not isinstance(value, str) or not value:
        raise RuntimeError("playback manifest must declare base_manifest")
...
def main() -> int:
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("playback patch-series manifest must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)

    base_destination = destination.parent / "base-attic-area-case-files-src"
    ...
    stage_base = Path(__file__).resolve().with_name("stage_attic_area_case_files.py")
    try:
        subprocess.run(
            [
                sys.executable,
        ...
        for item in manifest.get("overrides") or []:
            if not isinstance(item, dict):
                raise RuntimeError("each playback override must be an object")
    ...
    print(
        f"Staged Glulx Attic Playback Release {manifest.get('release')} "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_attic_playback: {exc}", file=sys.stderr)

#!/usr/bin/env python3
"""Stage the Release 1212 reactive-surface layer over Tara's pinned tree."""
...
def validate_base_manifest(manifest: dict[str, Any], manifest_path: Path) -> dict[str, Any]:
    """Verify exact derivation from the qualified Release 1211 manifest."""
    value = manifest.get("base_manifest")
    if not isinstance(value, str) or not value:
        raise RuntimeError("reactive-surface manifest must declare base_manifest")
...
def main() -> int:
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("reactive-surface patch-series manifest must contain an object")
    ...
    try:
        ...
        for item in manifest.get("overrides") or []:
            if not isinstance(item, dict):
                raise RuntimeError("each reactive-surface override must be an object")
    ...
    print(
        f"Staged reactive-surface Release {manifest.get('release')} from base "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_reactive_surface: {exc}", file=sys.stderr)
        raise SystemExit(2)

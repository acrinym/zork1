#!/usr/bin/env python3
"""Stage focused non-dam material consequences over qualified Release 1216."""
...
def validate_base_manifest(
    ...
    """Verify exact derivation from the qualified Release 1216 manifest."""
    value = manifest.get("base_manifest")
    if not isinstance(value, str) or not value:
        raise RuntimeError("material-consequences manifest must declare base_manifest")
...
def main() -> int:
    """Stage the exact base, apply the reviewed material delta, and emit a receipt."""
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("material-consequences patch-series manifest must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)

    base_destination = destination.parent / "base-ritual-resonance-src"
    ...
    stage_base = Path(__file__).resolve().with_name("stage_ritual_resonance.py")
    try:
        subprocess.run(
            [
                sys.executable,
        ...
        for item in manifest.get("overrides") or []:
            if not isinstance(item, dict):
                raise RuntimeError("each material-consequences override must be an object")
    ...
    remove_destination(base_destination)
    print(
        f"Staged Glulx material consequences Release {manifest.get('release')} over "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_material_consequences: {exc}", file=sys.stderr)

#!/usr/bin/env python3
"""Stage House Correspondence and Visitors over qualified Release 1222."""
...
def validate_base_manifest(
    ...
    if not isinstance(value, str) or not value:
        raise RuntimeError("Correspondence manifest must declare base_manifest")
...
def main() -> int:
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("Correspondence patch-series manifest must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)

    base_destination = destination.parent / "base-house-cellar-threshold-src"
    ...
    stage_base = Path(__file__).resolve().with_name("stage_house_cellar_threshold.py")
    try:
        subprocess.run(
            [
                sys.executable,
        ...
        for item in manifest.get("overrides") or []:
            if not isinstance(item, dict):
                raise RuntimeError("each correspondence override must be an object")
    ...
    remove_destination(base_destination)
    print(
        f"Staged Glulx House Correspondence and Visitors Release {manifest.get('release')} "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_house_correspondence_visitors: {exc}", file=sys.stderr)

#!/usr/bin/env python3
"""Stage Attic Area Case Files over qualified Release 1225."""
...
def validate_base_manifest(
    ...
    if not isinstance(value, str) or not value:
        raise RuntimeError("area case-file manifest must declare base_manifest")
...
def main() -> int:
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("area case-file patch-series manifest must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)

    base_destination = destination.parent / "base-attic-npc-dossiers-src"
    ...
    stage_base = Path(__file__).resolve().with_name("stage_attic_npc_dossiers.py")
    try:
        subprocess.run(
            [
                sys.executable,
        ...
        for item in manifest.get("overrides") or []:
            if not isinstance(item, dict):
                raise RuntimeError("each area case-file override must be an object")

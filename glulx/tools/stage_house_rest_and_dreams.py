#!/usr/bin/env python3
"""Stage House Rest and Dreams over qualified Release 1227."""
...
def validate_base_manifest(
    ...
    if not isinstance(value, str) or not value:
        raise RuntimeError("rest manifest must declare base_manifest")
...
def main() -> int:
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("rest patch-series manifest must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)

    base_destination = destination.parent / "base-attic-playback-src"
    ...
    stage_base = Path(__file__).resolve().with_name("stage_attic_playback.py")
    try:
        subprocess.run(
            [
                sys.executable,
        ...
        for item in manifest.get("overrides") or []:
            if not isinstance(item, dict):
                raise RuntimeError("each rest override must be an object")
    ...
    print(
        f"Staged Glulx House Rest and Dreams Release {manifest.get('release')} "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_house_rest_and_dreams: {exc}", file=sys.stderr)

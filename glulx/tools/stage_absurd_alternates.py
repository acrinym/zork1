#!/usr/bin/env python3
"""Stage Glulx absurd-alternate parity over qualified Shadow Logic Release 1213."""
...
def validate_base_manifest(manifest: dict[str, Any], manifest_path: Path) -> tuple[Path, dict[str, Any]]:
    """Verify exact derivation from the qualified Release 1213 manifest."""
    value = manifest.get("base_manifest")
    if not isinstance(value, str) or not value:
        raise RuntimeError("absurd-alternate manifest must declare base_manifest")
...
def main() -> int:
    """Stage the exact base, apply the parity delta, and emit a deterministic receipt."""
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("absurd-alternate patch-series manifest must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)

    base_destination = destination.parent / "base-shadow-logic-src"
    ...
    stage_shadow = Path(__file__).resolve().with_name("stage_shadow_logic.py")
    try:
        subprocess.run(
            [
                sys.executable,
        ...
        for item in manifest.get("overrides") or []:
            if not isinstance(item, dict):
                raise RuntimeError("each absurd-alternate override must be an object")
    ...
    remove_destination(base_destination)
    print(
        f"Staged Glulx absurd-alternate Release {manifest.get('release')} over "
        f"Shadow Logic Release {base_manifest.get('release')}; changed "
        f"{len(changed)} reviewed path(s)."
    )
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_absurd_alternates: {exc}", file=sys.stderr)

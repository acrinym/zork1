#!/usr/bin/env python3
"""Stage the House Kitchen Laboratory over qualified Release 1220."""
...
def validate_base_manifest(
    ...
    """Verify exact derivation from the qualified Release 1220 manifest."""
    value = manifest.get("base_manifest")
    if not isinstance(value, str) or not value:
        raise RuntimeError("Kitchen manifest must declare base_manifest")
...
def main() -> int:
    """Stage the exact base, apply the reviewed Kitchen delta, and emit a receipt."""
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("Kitchen patch-series manifest must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)

    base_destination = destination.parent / "base-living-room-museum-src"
    ...
    stage_base = Path(__file__).resolve().with_name("stage_living_room_museum.py")
    try:
        subprocess.run(
            [
                sys.executable,
        ...
        for item in manifest.get("overrides") or []:
            if not isinstance(item, dict):
                raise RuntimeError("each Kitchen override must be an object")
    ...
    remove_destination(base_destination)
    print(
        f"Staged Glulx House Kitchen Laboratory Release {manifest.get('release')} over "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_house_kitchen_laboratory: {exc}", file=sys.stderr)

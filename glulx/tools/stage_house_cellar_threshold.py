#!/usr/bin/env python3
"""Stage the House Cellar Threshold over qualified Release 1221."""
...
def validate_base_manifest(
    ...
    """Verify exact derivation from the qualified Release 1221 manifest."""
    value = manifest.get("base_manifest")
    if not isinstance(value, str) or not value:
        raise RuntimeError("Cellar manifest must declare base_manifest")
...
def main() -> int:
    """Stage the exact base, apply the reviewed Cellar delta, and emit a receipt."""
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("Cellar patch-series manifest must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)

    base_destination = destination.parent / "base-house-kitchen-laboratory-src"
    ...
    stage_base = Path(__file__).resolve().with_name("stage_house_kitchen_laboratory.py")
    try:
        subprocess.run(
            [
                sys.executable,
        ...
        for item in manifest.get("overrides") or []:
            if not isinstance(item, dict):
                raise RuntimeError("each Cellar override must be an object")
    ...
    remove_destination(base_destination)
    print(
        f"Staged Glulx House Cellar Threshold Release {manifest.get('release')} over "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_house_cellar_threshold: {exc}", file=sys.stderr)

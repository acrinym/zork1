#!/usr/bin/env python3
"""Stage Completed Expedition Archive over qualified Release 1229."""
...
def validate_base_manifest(
    ...
    if not isinstance(value, str) or not value:
        raise RuntimeError("completed expedition manifest must declare base_manifest")
...
def main() -> int:
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("completed expedition patch-series manifest must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)

    base_destination = destination.parent / "base-house-vulnerability-src"
    ...
    stage_base = Path(__file__).resolve().with_name("stage_house_vulnerability.py")
    try:
        subprocess.run(
            [
                sys.executable,
        ...
        for item in manifest.get("overrides") or []:
            if not isinstance(item, dict):
                raise RuntimeError("each completed expedition override must be an object")
    ...
    print(
        f"Staged Glulx Completed Expedition Archive Release {manifest.get('release')} "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_completed_expedition_archive: {exc}", file=sys.stderr)

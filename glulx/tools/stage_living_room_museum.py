#!/usr/bin/env python3
"""Stage the Living Room museum over qualified Release 1219."""
...
def validate_base_manifest(
    ...
    """Verify exact derivation from the qualified Release 1219 manifest."""
    value = manifest.get("base_manifest")
    if not isinstance(value, str) or not value:
        raise RuntimeError("museum manifest must declare base_manifest")
...
def main() -> int:
    """Stage the exact base, apply the reviewed museum delta, and emit a receipt."""
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("museum patch-series manifest must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)

    base_destination = destination.parent / "base-house-state-src"
    ...
    stage_base = Path(__file__).resolve().with_name("stage_house_state_foundation.py")
    try:
        subprocess.run(
            [
                sys.executable,
        ...
        for item in manifest.get("overrides") or []:
            if not isinstance(item, dict):
                raise RuntimeError("each museum override must be an object")
    ...
    remove_destination(base_destination)
    print(
        f"Staged Glulx Living Room museum Release {manifest.get('release')} over "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_living_room_museum: {exc}", file=sys.stderr)

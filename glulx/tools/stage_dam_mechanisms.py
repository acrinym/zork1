#!/usr/bin/env python3
"""Stage focused Glulx dam mechanisms over qualified Release 1214."""
...
def validate_base_manifest(
    ...
    """Verify exact derivation from the qualified Release 1214 manifest."""
    value = manifest.get("base_manifest")
    if not isinstance(value, str) or not value:
        raise RuntimeError("dam-mechanisms manifest must declare base_manifest")
...
def main() -> int:
    """Stage the exact base, apply the dam delta, and emit a receipt."""
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("dam-mechanisms patch-series manifest must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)

    base_destination = destination.parent / "base-absurd-alternates-src"
    ...
    stage_base = Path(__file__).resolve().with_name("stage_absurd_alternates.py")
    try:
        subprocess.run(
            [
                sys.executable,
        ...
        for item in manifest.get("overrides") or []:
            if not isinstance(item, dict):
                raise RuntimeError("each dam-mechanisms override must be an object")
    ...
    remove_destination(base_destination)
    print(
        f"Staged Glulx dam mechanisms Release {manifest.get('release')} over "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_dam_mechanisms: {exc}", file=sys.stderr)

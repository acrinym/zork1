#!/usr/bin/env python3
"""Stage Attic Archive Core over qualified Release 1223."""
...
def validate_base_manifest(
    ...
    if not isinstance(value, str) or not value:
        raise RuntimeError("Attic archive manifest must declare base_manifest")
...
def main() -> int:
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("Attic archive patch-series manifest must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)

    base_destination = destination.parent / "base-house-correspondence-visitors-src"
    ...
    stage_base = Path(__file__).resolve().with_name(
        "stage_house_correspondence_visitors.py"
    )
    try:
        subprocess.run(
            [
        ...
        for item in manifest.get("overrides") or []:
            if not isinstance(item, dict):
                raise RuntimeError("each Attic archive override must be an object")
    ...
    remove_destination(base_destination)
    print(
        f"Staged Glulx Attic Archive Core Release {manifest.get('release')} "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_attic_archive_core: {exc}", file=sys.stderr)

...
CheapGlk writes `r1279.sav` next to the story file. The save/restore gate must use that directory as cwd. Restore both binaries from the **reference** save bytes so a second SAVE cannot overwrite the file used for the restore comparison.

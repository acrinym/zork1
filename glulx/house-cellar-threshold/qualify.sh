...
assert layer['expected_artifact'] == {
    'file': os.environ['CELLAR_FILE'],
    'glulx_version_hex': os.environ['CELLAR_GLULX_VERSION'],
    'size_bytes': int(os.environ['CELLAR_SIZE']),
    'checksum_hex': os.environ['CELLAR_CHECKSUM'],
    'sha256': os.environ['CELLAR_SHA256'],
}
PY
...
assert not any(token in production for token in ('CSETUP', 'CINTRUDE', 'CCLEAR', 'CWATER', 'CREPORT', 'CMUTATE'))
PY
...
"$GLAZER_BIN" "$ASSEMBLY" -o "$BUILD/$CELLAR_FILE" \
  2>&1 | tee "$BUILD/glazer-assemble.log"
python glulx/tools/verify_ulx.py "$BUILD/$CELLAR_FILE" \
...
artifact = Path('glulx/build/house-cellar-threshold') / os.environ['CELLAR_FILE']
assert story['file'] == os.environ['CELLAR_FILE']
assert story['version_hex'] == os.environ['CELLAR_GLULX_VERSION']
assert story['size_bytes'] == int(os.environ['CELLAR_SIZE'])
assert story['checksum_hex'] == os.environ['CELLAR_CHECKSUM']
assert story['checksum_valid'] is True
assert story['sha256'] == os.environ['CELLAR_SHA256']
assert artifact.stat().st_size == int(os.environ['CELLAR_SIZE'])
assert hashlib.sha256(artifact.read_bytes()).hexdigest() == os.environ['CELLAR_SHA256']
PY
...
Path('glulx/build/house-cellar-threshold/QUALIFICATION-RECEIPT.json').write_text(

#!/usr/bin/env python3
"""Stage Living Zork Consequences over qualified Release 1235."""
...
def validate_base_manifest(
    ...
    """Resolve and validate the exact Release 1235 base contract."""
    value = manifest.get("base_manifest")
    if not isinstance(value, str) or not value:
        raise RuntimeError("living consequences manifest must declare base_manifest")
    ...
    try:
        ...
        raise RuntimeError(
            f"living consequences base manifest escaped Glulx lineage root: {base_path}"
        ) from exc
    if not base_path.is_file():
        raise RuntimeError(f"living consequences base manifest is missing: {base_path}")
    ...
    if base.get("release") != manifest.get("base_release"):
        raise RuntimeError("living consequences base release drift")
    if actual_sha != manifest.get("base_artifact_sha256"):
        raise RuntimeError("living consequences base artifact drift")
    return base_path, base


def main() -> int:
    """Stage the exact Great Canyon consequence delta and write a receipt."""
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("living consequences patch-series must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)

    base_destination = destination.parent / f"base-cuisine-src-{uuid.uuid4().hex}"
    ...
    stage_base = Path(__file__).resolve().parents[1] / "cuisine-hunger-stamina" / "stage.py"
    try:
        subprocess.run(
            [
                sys.executable,
    ...
    print(
        f"Staged Living Consequences Release {manifest.get('release')} over "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_living_zork_consequences: {exc}", file=sys.stderr)

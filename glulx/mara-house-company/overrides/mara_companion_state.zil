...
<ROUTINE MARA-RESET-STATE ()
    ...
    <MARA-PUT ,MARA-SLOT-HOUSE-INVITED 0>
    <MARA-PUT ,MARA-SLOT-PACK-RETRIEVED 0>
    <MARA-PUT ,MARA-SLOT-HOUSE-STAY 0>
    <MARA-PUT ,MARA-SLOT-MEAL-SHARED 0>
    <RTRUE>>

<ROUTINE MARA-ENSURE ()
    <COND (<EQUAL? <GET ,MARA-STATE 0> ,MARA-PREVIOUS-SCHEMA>
           ;"Release 1243 saves keep every witnessed event. Only the new
             House-company fields begin empty."
           <PUT ,MARA-STATE 0 ,MARA-SCHEMA>
           <MARA-PUT ,MARA-SLOT-HOUSE-INVITED 0>
           <MARA-PUT ,MARA-SLOT-PACK-RETRIEVED 0>
           <MARA-PUT ,MARA-SLOT-HOUSE-STAY 0>
           <MARA-PUT ,MARA-SLOT-MEAL-SHARED 0>)
          (<NOT <EQUAL? <GET ,MARA-STATE 0> ,MARA-SCHEMA>>
           <MARA-RESET-STATE>)>
...
<ROUTINE MARA-MEET ()
    ...
    <COND (<ZERO? <MARA-GET ,MARA-SLOT-MET>>
          ...
          (<MARA-GET ,MARA-SLOT-MEAL-SHARED>
           <TELL "Mara looks around the House as someone who has begun using it rather than visiting it. We have a Dam record, a moved camp, and one meal that actually existed, she says. That is enough history to stop calling this accidental company." CR>)
          (<MARA-GET ,MARA-SLOT-HOUSE-STAY>
           <TELL "Mara glances toward the Attic where she chose to leave her field pack. I am using the House as a base, she says. Using is not owning, and staying is not surrendering my route." CR>)
...
<ROUTINE MARA-ABOUT (TOPIC)
    ...
    <COND (<EQUAL? .TOPIC ,MARA-SURVEY-TOPIC ,MAP>
          ...
          (<EQUAL? .TOPIC ,MARA-COMPANY-TOPIC>
           <COND (<MARA-GET ,MARA-SLOT-MEAL-SHARED>
                  <TELL "We have worked a dangerous mechanism, carried my camp from one base to another, and split food neither of us could eat twice, Mara says. That is friendship with weight in the world. I prefer it to a title." CR>)
                 (<MARA-GET ,MARA-SLOT-HOUSE-STAY>
                  <TELL "Company changes when someone chooses to leave part of a working life at your base, Mara says. My pack is upstairs because I put it there, not because you unlocked a companion slot." CR>)
                 (<MARA-GET ,MARA-SLOT-DAM-SURVEY>
          ...
          (<EQUAL? .TOPIC ,WHITE-HOUSE>
           <COND (<MARA-GET ,MARA-SLOT-MEAL-SHARED>
                  <TELL "The House has crossed from claim to evidence, Mara says. My pack is in the Attic by consent, and the Kitchen has produced one meal we both actually ate. That is a base, not scenery." CR>)
                 (<MARA-GET ,MARA-SLOT-HOUSE-STAY>
                  <TELL "The Attic is dry, defensible, and private enough for my pack, Mara says. I chose to use it. That choice can be changed later without rewriting the fact that I made it." CR>)
                 (<MARA-GET ,MARA-SLOT-HOUSE-INVITED>
                  <TELL "The invitation is real, Mara says, but my field pack is still at the Dam. If this is going to become a base instead of a sentence, we retrieve the life I already have rather than conjuring a new one here." CR>)

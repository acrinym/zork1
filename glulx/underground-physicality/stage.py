#!/usr/bin/env python3
"""Apply Release 1249 Underground Sensory Physicality to the locked Release 1248 base."""
...
def validate_manifest(manifest: dict[str, Any], manifest_path: Path) -> dict[str, Any]:
    """Validate the declared locked Release 1248 base manifest and return it."""
    base_value = manifest.get("base_manifest")
    if not isinstance(base_value, str) or not base_value:
        raise RuntimeError("Release 1249 must declare its Release 1248 base manifest")
    ...
    if not isinstance(base, dict) or base.get("release") != manifest.get("base_release"):
        raise RuntimeError("Release 1249 base release drift")
    if not isinstance(artifact, dict) or artifact.get("locked") is not True:
        raise RuntimeError("Release 1249 requires a locked Release 1248 artifact")
    if artifact.get("sha256") != manifest.get("base_artifact_sha256"):
        raise RuntimeError("Release 1249 base artifact drift")
    return base
...
def validate_base_source(
    ...
    """Bind *base_source* to the immutable Release 1248 production/dev source identity."""
    ...
    if not isinstance(identities, dict):
        raise RuntimeError("Release 1249 must pin Release 1248 staged source identities")
    profile = "dev" if bool(receipt.get("dev_mode")) else "production"
    expected = identities.get(profile)
    if not isinstance(expected, str) or not expected:
        raise RuntimeError(f"Release 1249 has no pinned Release 1248 {profile} source identity")
    actual = source_identity(base_source)
    if actual != expected:
        raise RuntimeError(
            f"Release 1248 {profile} source identity drift: expected {expected}, got {actual}"
        )
    return receipt


def main() -> int:
    """Stage Release 1249 and record an exact source-change receipt."""
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("Release 1249 patch-series must contain an object")
    ...
    print(
        f"Staged Underground Sensory Physicality Release {manifest.get('release')} "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_underground_physicality: {exc}", file=sys.stderr)

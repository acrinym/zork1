#!/usr/bin/env python3
"""Stage the Cellar recovery locker over qualified Release 1237."""
...
def validate_base_manifest(
    ...
    if not isinstance(value, str) or not value:
        raise RuntimeError("recovery locker manifest must declare base_manifest")
    ...
    try:
        ...
        raise RuntimeError(
            f"recovery locker base manifest escaped Glulx lineage root: {base_path}"
        ) from exc
    if not base_path.is_file():
        raise RuntimeError(f"recovery locker base manifest is missing: {base_path}")
    ...
    if base.get("release") != manifest.get("base_release"):
        raise RuntimeError("recovery locker base release drift")
    if actual_sha != manifest.get("base_artifact_sha256"):
        raise RuntimeError("recovery locker base artifact drift")
...
def main() -> int:
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("recovery locker patch-series must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)

    base_destination = destination.parent / f"base-veteran-src-{uuid.uuid4().hex}"
    ...
    print(
        f"Staged Cellar Recovery Locker Release {manifest.get('release')} over "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_cellar_recovery_locker: {exc}", file=sys.stderr)

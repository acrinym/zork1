#!/usr/bin/env python3
"""Apply Release 1269 Structural Difficulty Modes to locked Release 1268."""
...
def validate_manifest(manifest: dict[str, Any], manifest_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(base_value, str) or not base_value:
        raise RuntimeError("Release 1269 must declare its Release 1268 base manifest")
    ...
    if not isinstance(base, dict) or base.get("release") != manifest.get("base_release"):
        raise RuntimeError("Release 1269 base release drift")
    if not isinstance(artifact, dict) or artifact.get("locked") is not True:
        raise RuntimeError("Release 1269 requires a locked Release 1268 artifact")
    if artifact.get("sha256") != manifest.get("base_artifact_sha256"):
        raise RuntimeError("Release 1269 base artifact drift")
    return base
...
def validate_base_source(base_source: Path, base_release: Any, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if expected != actual:
        raise RuntimeError(
            f"Release 1268 {profile} source identity drift: expected {expected}, got {actual}"
        )
    return receipt
...
def main() -> int:
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("Release 1269 patch-series must contain an object")
    ...
    for name in manifest.get("added_files") or []:
        ...
        if rel.is_absolute() or ".." in rel.parts or target.exists() or not source.is_file():
            raise RuntimeError(f"invalid Release 1269 added file: {name}")
        shutil.copy2(source, target)

    final = inventory(destination)
    changed = {path for path in set(base_files) | set(final) if base_files.get(path) != final.get(path)}
    ...
    print(
        f"Staged Structural Difficulty Release {manifest.get('release')} over Release "
        f"{base_manifest.get('release')}; changed {len(changed)} path(s)."
    )
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_structural_difficulty: {exc}", file=sys.stderr)

#!/usr/bin/env python3
"""Apply Release 1307 over an exact staged Release 1306 source tree."""
...
def validate_manifest(manifest: dict[str, Any], manifest_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(rel, str) or not rel:
        raise RuntimeError("Release 1307 must declare Release 1306 base manifest")
    ...
    if not isinstance(base, dict) or base.get("release") != 1306 or manifest.get("base_release") != 1306:
        raise RuntimeError("Release 1307 base release drift")
    if not isinstance(art, dict) or art.get("locked") is not True:
        raise RuntimeError("Release 1307 requires locked Release 1306 artifact")
    if art.get("sha256") != manifest.get("base_artifact_sha256"):
        raise RuntimeError("Release 1307 base artifact drift")
    return base
...
def validate_base_source(src: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(expected, str) or not expected:
        raise RuntimeError(f"Release 1307 missing locked 1306 {profile} source identity")
    if actual != expected:
...
def main() -> int:
    ...
    for name in manifest.get("added_files") or []:
        ...
        if rel.is_absolute() or ".." in rel.parts or target.exists() or not source.is_file():
            raise RuntimeError(f"invalid Release 1307 added file: {name}")
    ...
    print(f"Staged Release 1307 over locked Release 1306; changed {len(changed)} path(s).")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_release1307: {exc}", file=sys.stderr)

...
"$GLAZER_BIN" "$ASSEMBLY" -o "$BUILD/zork1-glulx-house-correspondence-visitors.ulx" 2>&1 | tee "$BUILD/glazer-assemble.log"
python glulx/tools/verify_ulx.py "$BUILD/zork1-glulx-house-correspondence-visitors.ulx" --json "$BUILD/story-report.json"
...
Path('glulx/build/house-correspondence-visitors/DISCOVERY-RECEIPT.json').write_text(

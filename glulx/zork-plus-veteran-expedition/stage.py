#!/usr/bin/env python3
"""Stage Zork Plus Veteran Expedition over qualified Release 1236."""
...
def validate_base_manifest(
    ...
    """Resolve and validate the exact Release 1236 base contract."""
    value = manifest.get("base_manifest")
    if not isinstance(value, str) or not value:
        raise RuntimeError("veteran expedition manifest must declare base_manifest")
    ...
    try:
        ...
        raise RuntimeError(
            f"veteran expedition base manifest escaped Glulx lineage root: {base_path}"
        ) from exc
    if not base_path.is_file():
        raise RuntimeError(f"veteran expedition base manifest is missing: {base_path}")
    ...
    if base.get("release") != manifest.get("base_release"):
        raise RuntimeError("veteran expedition base release drift")
    if actual_sha != manifest.get("base_artifact_sha256"):
        raise RuntimeError("veteran expedition base artifact drift")
    return base_path, base


def main() -> int:
    """Stage the exact Veteran Expedition delta and write a receipt."""
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("veteran expedition patch-series must contain an object")
    base_manifest_path, base_manifest = validate_base_manifest(manifest, manifest_path)

    base_destination = destination.parent / f"base-living-src-{uuid.uuid4().hex}"
    ...
    stage_base = Path(__file__).resolve().parents[1] / "living-zork-consequences" / "stage.py"
    try:
        subprocess.run(
            [
                sys.executable,
    ...
    print(
        f"Staged Zork Plus Release {manifest.get('release')} over "
...
if __name__ == "__main__":
    try:
        ...
        print(f"stage_zork_plus_veteran_expedition: {exc}", file=sys.stderr)
